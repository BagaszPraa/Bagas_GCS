...
QString FirmwarePlugin::vehicleImageOpaque(const Vehicle*) const
{
    ...
    //gambar pesawat dalam map
    // return QStringLiteral("/qmlimages/vehicleLCCM.svg");
}
...
void FirmwarePlugin::checkIfIsLatestStable(Vehicle* vehicle)
{
    ...
    if (qgcApp()->runningUnitTests()) {
        qCDebug(FirmwarePluginLog) << "Melewati Pemeriksaan Versi";
        return;
    }
    QString versionFile = _getLatestVersionFileUrl(vehicle);
    qCDebug(FirmwarePluginLog) << "Mengunduh" << versionFile;
    ...
    connect(
        ...
        [vehicle, this](QString remoteFile, QString localFile, QString errorMsg) {
            if (errorMsg.isEmpty()) {
                ...
                qCDebug(FirmwarePluginLog) << "Gagal mengunduh file versi FW terbaru.Kesalahan: " << errorMsg;
            }
            sender()->deleteLater();
        });
...
{
    qCDebug(FirmwarePluginLog) << "Unduh selesai" << remoteFile << localFile;
    // Now read the version file and pull out the version string
    QFile versionFile(localFile);
    if (!versionFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qCWarning(FirmwarePluginLog) << "Kesalahan membuka file versi yang diunduh.";
        return;
    }
    ...
    qCDebug(FirmwarePluginLog) << "Mencari nomor versi ...";

    if (match.hasMatch()) {
        version = match.captured(1);
    } else {
        qCWarning(FirmwarePluginLog) << "Tidak dapat parse info versi dari file" << remoteFile;
        return;
    }

    qCDebug(FirmwarePluginLog) << "Versi stabil terbaru = "  << version;
